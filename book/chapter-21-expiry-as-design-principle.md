...
Some decisions are genuinely complex. The assertion that every question can be resolved within a fourteen-day window is not one this book makes. The 14-Day Decision Aging Rule is a diagnostic threshold — the point at which the absence of resolution becomes visible as a structural signal — not a universal window for every question in the governance architecture.
...
Without expiry, ownership is nominal — the holder is named but has no designed horizon within which to discharge the holding. Without expiry, the decision surface has no designed close — questions can accumulate at any altitude indefinitely without the escalation trigger that would move them to the altitude where they can be resolved. Without expiry, governance terminates only when external pressure forces it — which means governance is not terminating at all. It is deferring.
